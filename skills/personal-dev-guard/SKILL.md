...
- 测试优先通过公开行为和稳定边界验证，不要为了访问内部状态，专门给生产代码增加只供测试使用的方法、接口、导出符号、setter/getter、hook，或放宽字段和方法可见性。
- 如果内部逻辑确实需要独立测试，先判断它是否代表真实、稳定的职责边界；只有生产设计本身也因此更合理时才拆分或提升可见性，不能把“测试需要”当作制造生产 API 的理由。
- 为了可测试性调整设计时，调整后的生产代码必须在不考虑测试的情况下仍然自然、规范、符合项目职责边界；否则应改测试策略，而不是让业务代码迁就测试。
...
- 是否为了测试方便，在生产代码中增加了只供测试使用或职责不合理的方法、接口、导出符号、setter/getter、hook，或放宽了可见性？如果去掉测试需求，这些设计是否仍然合理？
- 未来的人接手会不会骂人？
